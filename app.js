document.addEventListener('DOMContentLoaded', () => {
  ...
  const game = async () => {
    ...
    const wordsArrayAll = await fetch('500-english-words.json').then(resp => resp.json())
    ...
    function createBoard() {  //მასივში არეული მონაცემებს იღებს, ქმნის ბარათებს და აწყობს ეკრანზე
      ...
      for (let i = 0; i < wordsArray.length; i++) {
        ...
        if (wordsArray[i].frontText) {  //თუ მასივის წევრს ქართული მნიშვნელობა აქვს, ბარათს საწყის ტექსტს აწერს
    ...
    function checkForMatch() {
      ...
      if (optionOneId === optionTwoId) {
        ...
        translated.textContent = wordsArray[optionTwoId].frontText + " - " + wordsArray[optionOneId].backText
      ...
      document.getElementById('dict-score').style.color = "hsl(120," + reversePointsByPercent + "," + reversePointsByPercent + ")";
